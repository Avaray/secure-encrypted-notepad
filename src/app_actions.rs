...
impl EditorApp {
    ...
    /// Batch convert .txt/.md files to encrypted .sed files
    pub(crate) fn batch_convert(&mut self) {
        // Step 1: Pick multiple .txt / .md files
        let files = rfd::FileDialog::new()
            .set_title("Select files to convert to .sed")
        ...
        let files = match files {
            ...
            _ => {
                self.log_info("Batch conversion cancelled (no files selected)");
                return;
            }
        };

        // Step 2: Pick keyfile
        let keyfile = rfd::FileDialog::new()
            .set_title("Select keyfile for batch encryption")
            .pick_file();

        let keyfile = match keyfile {
            Some(k) => k,
            None => {
                self.log_info("Batch conversion cancelled (no keyfile selected)");
                return;
            }
        };

        // Validate keyfile size (must be 256 bytes)
        match std::fs::metadata(&keyfile) {
            Ok(meta) if meta.len() != 256 => {
                self.status_message = "Error: Keyfile must be exactly 256 bytes".to_string();
                self.log_error("Batch conversion: keyfile is not 256 bytes");
                return;
            }
            Err(e) => {
                self.status_message = format!("Error reading keyfile: {}", e);
                self.log_error(format!("Batch conversion: {}", e));
                return;
            }
            _ => {}
        }
        ...
        for file_path in &files {
            ...
            // Output path: same directory, same name, .sed extension
            let output_path = file_path.with_extension("sed");

            // Encrypt
            match encrypt_file(&content, &keyfile, &output_path) {
                Ok(_) => {
                    self.log_info(format!("Converted: {} → {}", file_path.display(), output_path.display()));
                    success += 1;
                }
                Err(e) => {
                    self.log_error(format!("Failed to encrypt {}: {}", file_path.display(), e));
                    failed += 1;
                }
            }
        }
        ...
        self.log_info(format!(
            "Batch conversion finished: {} succeeded, {} failed out of {} files",
