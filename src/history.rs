...
use crate::crypto::{decrypt_file, encrypt_file, CryptoError};
...
///
/// SECURITY:
/// - Snapshot is encrypted with same keyfile as main file
...
pub fn create_snapshot(
    ...
    encrypt_file(content, keyfile_path, &snapshot_path)?;
...
///
/// SECURITY:
/// - Uses same keyfile as for encryption
/// - If keyfile is wrong, decryption fails
pub fn load_version(
    version_info: &VersionInfo,
    keyfile_path: &Path,
) -> Result<String, HistoryError> {
    let content = decrypt_file(keyfile_path, &version_info.file_path)?;
    Ok(content)
}
...
pub fn restore_version(
    ...
    let content = load_version(version_info, keyfile_path)?;

    // 2. Optionally: Create backup of current version
    if create_backup && base_file_path.exists() {
        // Load current content
        let current_content = decrypt_file(keyfile_path, base_file_path)?;
    ...
    encrypt_file(&content, keyfile_path, base_file_path)?;
...
mod tests {
    ...
    fn test_create_and_list_snapshots() {
        ...
        create_snapshot("Version 2", keyfile_path, base_file, None).unwrap();
        thread::sleep(Duration::from_secs(1));
    ...
    fn test_restore_version() {
        ...
        // Create snapshot
        let version = create_snapshot("Original content", keyfile_path, base_file, None).unwrap();

        // Load
        let content = load_version(&version, keyfile_path).unwrap();
