...
=======
- [ ] **Editor Link Highlighting**: Automatically highlight URLs in the text editor. Allow users to open these links in their default web browser by clicking them while holding the `CTRL` key. Use the existing link color defined in the Theme Editor UI.
>>>>>>> parent of 1af4edf (feat: implement history diff viewer)
