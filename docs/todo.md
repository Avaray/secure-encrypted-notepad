...
- [x] **Theme Editor Interaction Polish**: Disable text selection for color labels in the Theme Editor to allow scrolling by clicking and dragging on the labels (consistent with background interaction).
...
- [x] **File Tree Status Indicator Alignment**: Refine the position of the color dot (access indicator) in the file tree (expandable mode). Add a small left margin and reduce the right margin to better align it with the filename. Consider replacing the dot with a high-quality SVG icon in the future.
